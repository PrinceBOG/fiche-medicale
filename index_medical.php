                                            ...
                                            <div class="col-lg-12">
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table table-striped">
                                                        ...
                                                        <tbody>
                                                            <?php
                                                            while ($i < count($medicamentsArray)) {
                                                                echo "
                                  <td>$i+1</td>
                                  <td>$medicamentsArray[$i]</td>
                                  ";
                                                            }
                                                            ?>
                                                        </tbody>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane show active" id="solid-tab1">
                                <form action="#" method="post">
                                    ...
                                    <!-- <button type="submit" name="submit_val">Enregistrer</button> -->
...
echo 'uit:  ' . $medicamentsArray[0];

echo 'userttp: ' . $nom . "<br>";

if (isset($_POST['submit1'])) {
    if (isset($_POST['outer-group'])) {
        // Récupérez les données du champ "outer-group"
...
if (isset($_POST['nom']) and isset($_POST['prenom']) and isset($_POST['gs']) and isset($_POST['taille']) and isset($_POST['poids']) and isset($_POST['statut']) and isset($_POST['pers_urg']) and isset($_POST['medecin'])) {
    if (!empty($_POST['nom']) and !empty($_POST['prenom']) and !empty($_POST['gs']) and !empty($_POST['taille']) and !empty($_POST['poids']) and !empty($_POST['statut']) and !empty($_POST['pers_urg']) and !empty($_POST['medecin'])) {
    ...
    $admed = isset($_POST['admed']) ? $_POST['admed'] : 0;
    $transp = isset($_POST['transp']) ? $_POST['transp'] : 0;
    $consul = isset($_POST['consul']) ? $_POST['consul'] : 0;
    ...
    if (isset($_POST['alg'])) {
        ...
        $algArray = array();
        foreach ($algInsts as $algInst) {
            $algValue = $algInst['alim'];
            $algArray[] = $algValue;
        }
        $charsAlim = implode(',', $algArray);
    }
}

/*if (isset($_POST['submit_cond'])) {
  if () {
    if (!empty($_POST['cond'])) {
      $conds = $_POST['cond'];
    }
...
try {
    ajouter($name, $prenom, $gs, $tal, $poids, $statut, $charscond, $charsMedoc, $charsPos, $charsFreq, $charsReaction, $charsAlim, $pers_urg, $medecin, $admed, $transp, $consul, $username, $password);
} catch (Exception $e) {
    echo 'Erreur: ' . $e->getMessage();
}

/*if (isset($_POST['soum2'])) {
  if (isset($_POST['nom_medoc']) and isset($_POST['pos']) and isset($_POST['freq'])) {
    $medocs = $_POST['nom_medoc'];
  }
  try {
    ajouter_med($medocs);
  } catch (Exception $e) {
...
if (isset($_POST['submit2'])) {
    ...
    try {
        ajouter_vis($etab, $datEnter, $heurEnter, $datSortie, $heurSortie, $medecin_vis, $etat_sante);
    } catch (Exception $e) {
        echo 'Erreur: ' . $e->getMessage();
    }
}
